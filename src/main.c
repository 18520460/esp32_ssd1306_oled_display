...
void ssd1306_init() {
	...
	i2c_master_write_byte(cmd, /*OLED_CMD_SET_SEGMENT_REMAP*/ 0xA0, true); // reverse left-right mapping
...
void task_ssd1306_display_text(char *arg_text) {
	...
	for (uint8_t i = 0; i < text_len; i++) {
        ...
        while(done != true){
            ...
            i2c_master_write_byte(cmd, column, true);  //column start addr
            i2c_master_write_byte(cmd, 127, true); //column end addr
            i2c_master_write_byte(cmd, 0x22, true);
            i2c_master_write_byte(cmd, page, true); // page start addr
            i2c_master_write_byte(cmd, 7, true); // page end addr
            i2c_master_stop(cmd);
            ...
            if(count == 0){
                i2c_master_write(cmd, font8x8_basic_tr[(uint8_t)text[i] - 48], 16, true);
                page++;
            }else{
                i2c_master_write(cmd, &font8x8_basic_tr[(uint8_t)text[i] - 48][16], 16, true);
                page--;
            } 
            i2c_master_stop(cmd);
            i2c_master_cmd_begin(I2C_NUM_0, cmd, 10/portTICK_PERIOD_MS);
...
void task_ssd1306_read_out(void *ignore){
    uint8_t *buffer = NULL;
...
void app_main(void)
{
    ...
    xTaskCreate(&task_ssd1306_display_text, "ssd1306_display_text",  2048,
		(void *)"18520460", 6, NULL);
    xTaskCreate(&task_ssd1306_read_out, "ssd1306_read",  2048, NULL, 6, NULL);
    ESP_LOGI(TAG, "FINISH");
